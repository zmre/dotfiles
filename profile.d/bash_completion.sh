...
if [ -n "${FUNCNAME:-}" ]; then
	...
	BASH_COMPLETION="$HOME/profile.d/bash_completion.sh" 2>/dev/null
	BASH_COMPLETION_DIR="$HOME/profile.d/bash_completion.d" 2>/dev/null
else
	declare -r BASH_COMPLETION="$HOME/profile.d/bash_completion.sh" 2>/dev/null
	declare -r BASH_COMPLETION_DIR="$HOME/profile.d/bash_completion.d" 2>/dev/null
fi
...
_aptitude()
{
	...
	for (( i=0; i < ${#COMP_WORDS[@]}-1; i++ )); do
	    if [[ ${COMP_WORDS[i]} == @(install|reinstall|hold|unhold|markauto|unmarkauto|dist-upgrade|download|forbid-version|purge|remove) ]]; then
		special=${COMP_WORDS[i]}
	    fi
...
[ -n "${have:-}" ] && complete -F _cdrecord $filenames cdrecord
...
[ "${have:-}" ] && complete -F _dd $nospace $filenames dd
...
[ -n "${have:-}" ] && complete -F _mkinitrd mkinitrd
...
_pkg_config()
{
	...
	if [[ "$cur" == -* ]]; then
		...
		COMPREPLY=( $( compgen -W '-version --modversion \
		      ...
		      --define-variable= --exists --uninstalles \
...
[ -n "${have:-}" ] && complete -F _pkg_config pkg-config
